...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define LSM6DSO_HP_DISABLE        0xFBU  /* Disable HP filter */
#define LSM6DSO_HP_ENABLE_DIV400  0xC4U  /* Enable HP filter, DIV/400 */
#define LSM6DSO_DEFAULT_ODR       417.0f /* Default output/batch data rate */
#define LSM6DSO_DEFAULT_FS        2      /* Default full scale */
...
uint8_t Init_Demo(void)
{
  ...
  {
    if (IKS01A3_MOTION_SENSOR_Read_Register(IKS01A3_LSM6DSO_0, LSM6DSO_CTRL8_XL, &data) != BSP_ERROR_NONE)
    {
      return 0;
    }

    data &= LSM6DSO_HP_DISABLE;

    if (IKS01A3_MOTION_SENSOR_Write_Register(IKS01A3_LSM6DSO_0, LSM6DSO_CTRL8_XL, data) != BSP_ERROR_NONE)
    {
      return 0;
    }
  }
  ...
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_BDR(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, LSM6DSO_DEFAULT_ODR) != BSP_ERROR_NONE)
  {
    return 0;
  }

  /* Set FIFO watermark level */
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_Watermark_Level(IKS01A3_LSM6DSO_0, MotionSP_Parameters.FftSize - 1) != BSP_ERROR_NONE)
  {
    return 0;
  }

  /* Set FIFO to stop on FIFO threshold */
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_Stop_On_Fth(IKS01A3_LSM6DSO_0, PROPERTY_ENABLE) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
uint8_t Restart_FIFO(void)
{
  ...
  /* FIFO Bypass Mode */
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_Mode(IKS01A3_LSM6DSO_0, LSM6DSO_BYPASS_MODE) != BSP_ERROR_NONE)
  {
    return 0;
  }

  /* FIFO Mode*/
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_Mode(IKS01A3_LSM6DSO_0, LSM6DSO_FIFO_MODE) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
uint8_t Set_ODR(uint8_t value)
{
  if ((value <= SensorSetting.odr_list[0]) && value != 0)
  {
    ...
    if (IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, SensorSetting.odr_list[value]) != BSP_ERROR_NONE)
    {
      return 0;
    }

    /* Set FIFO BDR (has to be equal to or lower than accelerometer ODR) */
    if (IKS01A3_MOTION_SENSOR_FIFO_Set_BDR(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, SensorSetting.odr_list[value]) != BSP_ERROR_NONE)
    {
      return 0;
    }
...
uint8_t Set_Full_Scale(uint8_t value)
{
  ...
  if (value <= SensorSetting.fs_list[0] && value != 0)
  {
    ...
    if (IKS01A3_MOTION_SENSOR_SetFullScale(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, SensorSetting.fs_list[value]) != BSP_ERROR_NONE)
    {
      return 0;
    }
...
static void Init_Sensors(void)
{
   ...
   *   - FS   = <-2g, 2g>
   */
  (void)IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, LSM6DSO_DEFAULT_ODR);
  (void)IKS01A3_MOTION_SENSOR_SetFullScale(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, LSM6DSO_DEFAULT_FS);
}
...
static uint8_t Collect_Data(void)
{
  ...
  if (FinishAvgFlag == 0 && fftIsEnabled == 0 && AccIntReceived == 1)
  {
    ...
    IKS01A3_MOTION_SENSOR_FIFO_Get_Num_Samples(IKS01A3_LSM6DSO_0, &samples_in_fifo);
    ...
    while (fftIsEnabled == 0)
    {
      ...
      IKS01A3_MOTION_SENSOR_FIFO_Get_Tag(IKS01A3_LSM6DSO_0, &tag);
      IKS01A3_MOTION_SENSOR_FIFO_Get_Axes(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, &acceleration);
...
static uint8_t Meas_Odr(void)
{
  ...
  if (IKS01A3_MOTION_SENSOR_DRDY_Set_Mode(IKS01A3_LSM6DSO_0, LSM6DSO_DRDY_PULSED) != BSP_ERROR_NONE)
  {
    return 0;
  }
  ...
  if (IKS01A3_MOTION_SENSOR_Disable(IKS01A3_LSM6DSO_0, MOTION_ACCELERO) != BSP_ERROR_NONE)
  {
    return 0;
  }
  ...
  if (IKS01A3_MOTION_SENSOR_DRDY_Set_Mode(IKS01A3_LSM6DSO_0, LSM6DSO_DRDY_LATCHED) != BSP_ERROR_NONE)
  {
    return 0;
  }

  /* Enable accelerometer */
  if (IKS01A3_MOTION_SENSOR_Enable(IKS01A3_LSM6DSO_0, MOTION_ACCELERO) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
static uint8_t Enable_DRDY(void)
{
  ...
  /* Enable DRDY */
  if (IKS01A3_MOTION_SENSOR_DRDY_Enable_Interrupt(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, IKS01A3_MOTION_SENSOR_INT1_PIN) != BSP_ERROR_NONE)
  {
    return 0;
  }

  /* Clear DRDY */
  if (IKS01A3_MOTION_SENSOR_GetAxesRaw(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, &axes) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
{
  /* Disable DRDY */
  if (IKS01A3_MOTION_SENSOR_DRDY_Disable_Interrupt(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, IKS01A3_MOTION_SENSOR_INT1_PIN) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
{
  /* Enable FIFO full flag interrupt */
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_INT1_FIFO_Full(IKS01A3_LSM6DSO_0, PROPERTY_ENABLE) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
static uint8_t Disable_FIFO(void)
{
  ...
  if (IKS01A3_MOTION_SENSOR_FIFO_Set_INT1_FIFO_Full(IKS01A3_LSM6DSO_0, PROPERTY_DISABLE) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
static uint8_t Get_Available_Sensors(void)
{
  ...
  SensorList[sensors_available] = IKS01A3_LSM6DSO_0;
