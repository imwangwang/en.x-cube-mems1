...
/**
 * @mainpage Documentation for MotionPW package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionPW software is an add-on for the X-CUBE-MEMS1 software and provides
 * the real-time step count, cadence and confidence.
...
static void PW_Data_Handler(TMsg *Msg);
 ...
 *         expansion board to detect the steps, cadence & confidence and send it from a Nucleo
...
int main(void)
{
  ...
  */
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionPW_manager_get_version(lib_version, &lib_version_len);
...
static void Init_Sensors(void)
{
   ...
   *   - FS   = <-8g, 8g>
   */
  (void)IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, 50.0f);
  (void)IKS01A3_MOTION_SENSOR_SetFullScale(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, 8);
}
...
static void PW_Data_Handler(TMsg *Msg)
{
  ...
  static MAW_output_t maw_data_out;
  static MPW_output_t mpw_data_out_prev = {.Cadence = 0, .Nsteps = 0, .Confidence = 0};
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    mpw_data_in.AccX = maw_data_in.AccX = (float)AccValue.x / 1000.0f;
    mpw_data_in.AccY = maw_data_in.AccY = (float)AccValue.y / 1000.0f;
    mpw_data_in.AccZ = maw_data_in.AccZ = (float)AccValue.z / 1000.0f;
    ...
    BSP_LED_On(LED2);
    MotionPW_manager_run(&mpw_data_in, &mpw_data_out);
    BSP_LED_Off(LED2);

    if (ProgramState == GUI_MODE)
    {
      Serialize_s32(&Msg->Data[55], (int32_t)mpw_data_out.Cadence, 4);
      Serialize_s32(&Msg->Data[59], (int32_t)mpw_data_out.Nsteps, 4);
      Serialize_s32(&Msg->Data[63], (int32_t)mpw_data_out.Confidence, 4);
    }
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].occurr = 0; /* 0 if new session */
        DataByte[DataIndex].cadence = mpw_data_out_prev.Cadence;
        DataByte[DataIndex].steps = mpw_data_out_prev.Nsteps;
        DataByte[DataIndex].confidence = mpw_data_out_prev.Confidence;
        DataIndex++;

        mpw_data_out_prev.Cadence = mpw_data_out.Cadence;
        mpw_data_out_prev.Nsteps = mpw_data_out.Nsteps;
        mpw_data_out_prev.Confidence = mpw_data_out.Confidence;
        return;
      }
      ...
      /* Data changed */
      if (mpw_data_out.Cadence != mpw_data_out_prev.Cadence || mpw_data_out.Nsteps != mpw_data_out_prev.Nsteps ||
          mpw_data_out.Confidence != mpw_data_out_prev.Confidence)
      {
        /* Store new data in buffer */
        RTC_Handler_DataStamp(&DataByte[DataIndex].date_time);
        DataByte[DataIndex].occurr = 1; /* 0 if new session */
        DataByte[DataIndex].cadence = mpw_data_out.Cadence;
        DataByte[DataIndex].steps = mpw_data_out.Nsteps;
        DataByte[DataIndex].confidence = mpw_data_out.Confidence;
        DataIndex++;
...
static void Pressure_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  {
    (void)IKS01A3_ENV_SENSOR_GetValue(Instance, ENV_PRESSURE, &press_value);
