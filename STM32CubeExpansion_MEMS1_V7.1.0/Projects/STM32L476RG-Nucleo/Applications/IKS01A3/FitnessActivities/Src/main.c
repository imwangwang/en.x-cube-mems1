...
/**
 * @mainpage Documentation for MotionFA package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionFA software is an add-on for the X-CUBE-MEMS1 software and provides
 * the real-time fitness activities recognition data.
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define ALGO_FREQ  25U /* Algorithm frequency [Hz] */
...
static IKS01A3_MOTION_SENSOR_Axes_t GyrValue;
...
static void FA_Data_Handler(TMsg *Msg);
 ...
 *         expansion board to count number of bices curls or squats and send it from a Nucleo
...
int main(void)
{
  ...
  */
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionFA_manager_get_version(lib_version, &lib_version_len);
    ...
    if (SensorReadRequest == 1U)
    {
      ...
      switch (ProgramState)
      {
        case GUI_MODE:
          ...
          /* Fitness Activities specific part */
          FA_Data_Handler(&msg_dat);
...
static void Init_Sensors(void)
{
   ...
   *   - ODR >= 25 Hz
   */
  (void)IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LSM6DSO_0, MOTION_ACCELERO, 25.0f);

  /* Set gyroscope:
   *   - ODR >= 25 Hz
   */
  (void)IKS01A3_MOTION_SENSOR_SetOutputDataRate(IKS01A3_LSM6DSO_0, MOTION_GYRO, 25.0f);

  /* Set pressure sensor:
   *   - ODR >= 25 Hz
   */
  (void)IKS01A3_ENV_SENSOR_SetOutputDataRate(IKS01A3_LPS22HH_0, ENV_PRESSURE, 25.0f);
}
...
/**
 * @brief  Fitness Activities data handler
 * @param  Msg the Fitness Activities data part of the stream
 * @retval None
 */
static void FA_Data_Handler(TMsg *Msg)
{
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    if ((SensorsEnabled & GYROSCOPE_SENSOR) == GYROSCOPE_SENSOR)
    {
      if ((SensorsEnabled & PRESSURE_SENSOR) == PRESSURE_SENSOR)
      {
        ...
        data_in.GyrX = (float)GyrValue.x / 1000.0f;
        data_in.GyrY = (float)GyrValue.y / 1000.0f;
        data_in.GyrZ = (float)GyrValue.z / 1000.0f;
        ...
        if (ProgramState == GUI_MODE)
        {
          Serialize_s32(&Msg->Data[55], (int32_t)(data_out.Counter), 4);
          Serialize_s32(&Msg->Data[59], (int32_t)(activity_type + 1), 4);
        }
        else if (ProgramState == STANDALONE_MODE)
        {
          ...
          if (SessionNew == 1U)
          {
            ...
            DataByte[DataIndex].activity_counter = 0; /* 0 if new session */
            DataByte[DataIndex].activity_type = 0;
            DataIndex++;
          ...
          if (data_out_prev.Counter != data_out.Counter)
          {
            ...
            RTC_Handler_DataStamp(&DataByte[DataIndex].date_time);
            DataByte[DataIndex].activity_counter = data_out.Counter; /* 0 if new session */
            DataByte[DataIndex].activity_type = activity_type + 1;
            DataIndex++;
...
static void Gyro_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  if ((SensorsEnabled & GYROSCOPE_SENSOR) == GYROSCOPE_SENSOR)
  {
    (void)IKS01A3_MOTION_SENSOR_GetAxes(Instance, MOTION_GYRO, &GyrValue);
...
static void Pressure_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  if ((SensorsEnabled & PRESSURE_SENSOR) == PRESSURE_SENSOR)
  {
    (void)IKS01A3_ENV_SENSOR_GetValue(Instance, ENV_PRESSURE, &PresValue);
