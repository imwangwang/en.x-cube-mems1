...
/**
 * @mainpage Documentation for MotionAT package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionAT software is an add-on for the X-CUBE-MEMS1 software and provides
...
static void AT_Data_Handler(TMsg *Msg);
 ...
 *         expansion board to detect the number of active seconds and send it from a Nucleo
    ...
    if (SensorReadRequest == 1U)
    {
      ...
      switch (ProgramState)
      {
        case GUI_MODE:
          ...
          /* Active Time specific part */
          AT_Data_Handler(&msg_dat);
...
/**
 * @brief  Active Time data handler
 * @param  Msg the Active Time data part of the stream
 * @retval None
 */
static void AT_Data_Handler(TMsg *Msg)
{
  MAT_input_t data_in = {.acc_x = 0.0f, .acc_y = 0.0f, .acc_z = 0.0f};
  static MAT_output_t data_out;
  static MAT_output_t data_out_prev = {.active = 0};
  static uint32_t session_counter = 0;
  static float active_seconds = 0.0f;
  static float active_seconds_prev = 0.0f;
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    BSP_LED_On(LED2);
    MotionAT_manager_run(&data_in, &data_out);
    BSP_LED_Off(LED2);

    /* Count active seconds */
    active_seconds += (float)data_out.active * 0.02f; /* 1/50Hz = 0.02s per 1 algorithm run */

    if (ProgramState == GUI_MODE)
    {
      Serialize_s32(&Msg->Data[55], (int32_t)data_out.active, 4);
      Serialize_s32(&Msg->Data[59], (int32_t)active_seconds, 4);
    }
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].session = 0; /* 0 if new session */
        DataByte[DataIndex].active = data_out.active;
        DataByte[DataIndex].active_seconds = (uint32_t)active_seconds;
        DataIndex++;

        data_out_prev.active = data_out.active;
        active_seconds_prev = active_seconds;
        return;
      }
      ...
      /* Data changed */
      if (data_out_prev.active != data_out.active || (uint32_t)active_seconds_prev != (uint32_t)active_seconds)
      {
        /* Store new data in buffer */
        RTC_Handler_DataStamp(&DataByte[DataIndex].date_time);
        DataByte[DataIndex].session = 1; /* 0 if new session */
        DataByte[DataIndex].active = data_out.active;
        DataByte[DataIndex].active_seconds = (uint32_t)active_seconds;
        DataIndex++;
