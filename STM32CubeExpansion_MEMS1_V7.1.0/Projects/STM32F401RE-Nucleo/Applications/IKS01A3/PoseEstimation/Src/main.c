...
/**
 * @mainpage Documentation for MotionPE package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionPE software is an add-on for the X-CUBE-MEMS1 software and provides
 * the real-time pose (standing, sitting, lying down) estimation.
...
static void PE_Data_Handler(TMsg *Msg);
 ...
 *         expansion board to recognize user pose data and send it from a Nucleo
...
int main(void)
{
  ...
  */
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionPE_manager_get_version(lib_version, &lib_version_len);
    ...
    if (SensorReadRequest == 1U)
    {
      ...
      switch (ProgramState)
      {
        case GUI_MODE:
          ...
          /* Pose Estimation specific part */
          PE_Data_Handler(&msg_dat);
...
/**
 * @brief  Pose Estimation data handler
 * @param  Msg the Pose Estimation data part of the stream
 * @retval None
 */
static void PE_Data_Handler(TMsg *Msg)
{
  ...
  static MPE_output_t data_out_prev = MPE_UNKNOWN;
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].pose_type = (uint8_t)MPE_UNKNOWN;
        DataIndex++;

        data_out_prev = MPE_UNKNOWN;
        return;
      }
