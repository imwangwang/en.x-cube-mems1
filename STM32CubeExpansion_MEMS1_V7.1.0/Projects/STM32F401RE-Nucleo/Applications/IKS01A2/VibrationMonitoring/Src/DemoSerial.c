...
static uint8_t Handle_SUBCMD(TMsg *Msg)
{
  ...
  switch (Msg->Data[3]) /* SUBCMD */
  {
    ...
    case SUBCMD_SET_SENSOR_INDEX:
      ...
      {
        if (sensor_list[Msg->Data[5]] != IKS01A2_LSM6DSL_0)
        {
...
static uint8_t Get_Actual_FS_Index(uint8_t *Value)
{
  ...
  if (IKS01A2_MOTION_SENSOR_GetFullScale(IKS01A2_LSM6DSL_0, MOTION_ACCELERO, &fullScale) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
static uint8_t Get_Actual_ODR_Index(uint8_t *Value)
{
  ...
  if (IKS01A2_MOTION_SENSOR_GetOutputDataRate(IKS01A2_LSM6DSL_0, MOTION_ACCELERO, &odr) != BSP_ERROR_NONE)
  {
    return 0;
  }
...
static uint8_t Get_Actual_Sensor_Index(uint8_t *Value)
{
  ...
  {
    if (IKS01A2_LSM6DSL_0 == sensor_list[i])
    {
      *Value = i;
      return 1;
    }
