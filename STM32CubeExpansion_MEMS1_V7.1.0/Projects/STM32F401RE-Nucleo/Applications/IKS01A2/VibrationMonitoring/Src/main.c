...
uint8_t Set_Full_Scale(uint8_t value)
{
  ...
  if (value <= SensorSetting.fs_list[0] && value != 0)
  {
    ...
    if (IKS01A2_MOTION_SENSOR_SetFullScale(IKS01A2_LSM6DSL_0, MOTION_ACCELERO, SensorSetting.fs_list[value]) != BSP_ERROR_NONE)
    {
      return 0;
    }
...
uint8_t Set_Samples(uint8_t value)
{
  ...
  if (value <= SensorSetting.samples_list[0] && value != 0)
  {
    ...
    /* Set FIFO watermark level */
    if (IKS01A2_MOTION_SENSOR_FIFO_Set_Watermark_Level(IKS01A2_LSM6DSL_0, (MotionSP_Parameters.FftSize + 1) * 3))
    {
      return 0;
    }
