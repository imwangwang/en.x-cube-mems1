...
/**
 * @mainpage Documentation for MotionSD package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 ...
 *         expansion board to to distinguish user working mode (standing vs sitting desk)
...
static void SD_Data_Handler(TMsg *Msg)
{
  ...
  static MSD_output_t data_out_prev = MSD_UNKNOWN_DESK;
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    if (ProgramState == STANDALONE_MODE)
    {
      HAL_Delay(1);
    }
    ...
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].position = (uint8_t)data_out;
        DataIndex++;

        data_out_prev = MSD_UNKNOWN_DESK;
        return;
      }
...
static void Pressure_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  {
    (void)IKS01A2_ENV_SENSOR_GetValue(Instance, ENV_PRESSURE, &PressValue);
