...
static void GPIO_Init(void)
{
  ...
  /* Interrupt pins X-NUCLEO-IKS01A2 */
  // LSM6DSL INT1                        PB5
  // LSM6DSL INT2                        PB4
  // LPS22HB INT1                        PB10
  // DIL24 INT2                          PC1
  // DIL24 INT1                          PC0 (initialized separately)
  // LSM303AGR DRDY/LSM303AGR INT        PA4
  // LSM303AGR INT/LSM303AGR DRDY        PB0
  // USER INT                            PA10
...
static void Enable_Disable_Sensors(void)
{
  if ((SensorsEnabled & ACCELEROMETER_SENSOR_ENABLED) == ACCELEROMETER_SENSOR_ENABLED)
  {
    (void)IKS01A2_MOTION_SENSOR_Enable(AccInstance, MOTION_ACCELERO);
  }
  else
  {
  ...
  if ((SensorsEnabled & GYROSCOPE_SENSOR_ENABLED) == GYROSCOPE_SENSOR_ENABLED)
  {
    (void)IKS01A2_MOTION_SENSOR_Enable(GyrInstance, MOTION_GYRO);
  }
  else
  {
    (void)IKS01A2_MOTION_SENSOR_Disable(GyrInstance, MOTION_GYRO);
  }
  if ((SensorsEnabled & MAGNETIC_SENSOR_ENABLED) == MAGNETIC_SENSOR_ENABLED)
  {
    (void)IKS01A2_MOTION_SENSOR_Enable(MagInstance, MOTION_MAGNETO);
  }
  else
  {
    (void)IKS01A2_MOTION_SENSOR_Disable(MagInstance, MOTION_MAGNETO);
  }
  if ((SensorsEnabled & HUMIDITY_SENSOR_ENABLED) == HUMIDITY_SENSOR_ENABLED)
  {
    (void)IKS01A2_ENV_SENSOR_Enable(HumInstance, ENV_HUMIDITY);
  }
  else
  {
    (void)IKS01A2_ENV_SENSOR_Disable(HumInstance, ENV_HUMIDITY);
  }
  if ((SensorsEnabled & TEMPERATURE_SENSOR_ENABLED) == TEMPERATURE_SENSOR_ENABLED)
  {
    (void)IKS01A2_ENV_SENSOR_Enable(TmpInstance, ENV_TEMPERATURE);
  }
  else
  {
    (void)IKS01A2_ENV_SENSOR_Disable(TmpInstance, ENV_TEMPERATURE);
  }
  if ((SensorsEnabled & PRESSURE_SENSOR_ENABLED) == PRESSURE_SENSOR_ENABLED)
  {
    (void)IKS01A2_ENV_SENSOR_Enable(PrsInstance, ENV_PRESSURE);
  }
  else
  {
    (void)IKS01A2_ENV_SENSOR_Disable(PrsInstance, ENV_PRESSURE);
  }
}
...
static void Time_Handler(TMsg *Msg)
{
  ...
  if (stop_time >= StartTime)
  {
    time_us = stop_time - StartTime;
  }
  else
  {
    time_us = stop_time - StartTime + 1000000; /* TIM counter overflew 1 second */
  }
...
static void Press_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  if (IKS01A2_ENV_SENSOR_Get_DRDY_Status(Instance, ENV_PRESSURE, &status) == BSP_ERROR_NONE && status == 1U)
  {
    NewData++;
    NewDataFlags |= 1U;

    (void)IKS01A2_ENV_SENSOR_GetValue(Instance, ENV_PRESSURE, &pressure);
...
static void Temp_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  if (Instance == IKS01A2_STTS22H_0)
  {
    (void)IKS01A2_ENV_SENSOR_GetOutputDataRate(Instance, ENV_TEMPERATURE, &odr);
  ...
  {
    if (IKS01A2_ENV_SENSOR_Get_DRDY_Status(Instance, ENV_TEMPERATURE, &status) == BSP_ERROR_NONE && status == 1U)
    {
      drdy = 1;
    }
    else
  ...
  if (drdy == 1)
  {
    ...
    (void)IKS01A2_ENV_SENSOR_GetValue(Instance, ENV_TEMPERATURE, &temperature);
...
static void Hum_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  if (IKS01A2_ENV_SENSOR_Get_DRDY_Status(Instance, ENV_HUMIDITY, &status) == BSP_ERROR_NONE && status == 1U)
  {
    NewData++;
    NewDataFlags |= 4U;

    (void)IKS01A2_ENV_SENSOR_GetValue(Instance, ENV_HUMIDITY, &humidity);
...
static void Gyro_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  if (IKS01A2_MOTION_SENSOR_Get_DRDY_Status(Instance, MOTION_GYRO, &status) == BSP_ERROR_NONE && status == 1U)
  {
    NewData++;
    NewDataFlags |= 16U;

    (void)IKS01A2_MOTION_SENSOR_GetAxes(Instance, MOTION_GYRO, &angular_velocity);
...
static void Magneto_Sensor_Handler(TMsg *Msg, uint32_t Instance)
{
  ...
  if (IKS01A2_MOTION_SENSOR_Get_DRDY_Status(Instance, MOTION_MAGNETO, &status) == BSP_ERROR_NONE && status == 1U)
  {
    NewData++;
    NewDataFlags |= 32U;

    (void)IKS01A2_MOTION_SENSOR_GetAxes(Instance, MOTION_MAGNETO, &magnetic_field);
...
static void MLC_Handler(TMsg *Msg)
{
  ...
  if ((AccInstance == IKS01A2_LSM6DSOX_0) && (GyrInstance == IKS01A2_LSM6DSOX_0))
  {
    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FUNC_CFG_ACCESS, LSM6DSOX_EMBEDDED_FUNC_BANK << 6);

    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC0_SRC, &mlc_status[0]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC1_SRC, &mlc_status[1]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC2_SRC, &mlc_status[2]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC3_SRC, &mlc_status[3]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC4_SRC, &mlc_status[4]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC5_SRC, &mlc_status[5]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC6_SRC, &mlc_status[6]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_MLC7_SRC, &mlc_status[7]);

    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FUNC_CFG_ACCESS, LSM6DSOX_USER_BANK << 6);
  }
  else
  {
    mlc_status[0] = 0;
...
static void FSM_Handler(TMsg *Msg)
{
  ...
  if ((AccInstance == IKS01A2_LSM6DSO_0) && (GyrInstance == IKS01A2_LSM6DSO_0))
  {
    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FUNC_CFG_ACCESS, LSM6DSO_EMBEDDED_FUNC_BANK << 6);

    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS1, &fsm_status[0]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS2, &fsm_status[1]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS3, &fsm_status[2]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS4, &fsm_status[3]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS5, &fsm_status[4]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS6, &fsm_status[5]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS7, &fsm_status[6]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS8, &fsm_status[7]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS9, &fsm_status[8]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS10, &fsm_status[9]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS11, &fsm_status[10]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS12, &fsm_status[11]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS13, &fsm_status[12]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS14, &fsm_status[13]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS15, &fsm_status[14]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FSM_OUTS16, &fsm_status[15]);

    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSO_0, LSM6DSO_FUNC_CFG_ACCESS, LSM6DSO_USER_BANK << 6);
  }
  else if ((AccInstance == IKS01A2_LSM6DSOX_0) && (GyrInstance == IKS01A2_LSM6DSOX_0))
  {
    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FUNC_CFG_ACCESS, LSM6DSOX_EMBEDDED_FUNC_BANK << 6);

    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS1, &fsm_status[0]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS2, &fsm_status[1]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS3, &fsm_status[2]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS4, &fsm_status[3]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS5, &fsm_status[4]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS6, &fsm_status[5]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS7, &fsm_status[6]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS8, &fsm_status[7]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS9, &fsm_status[8]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS10, &fsm_status[9]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS11, &fsm_status[10]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS12, &fsm_status[11]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS13, &fsm_status[12]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS14, &fsm_status[13]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS15, &fsm_status[14]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FSM_OUTS16, &fsm_status[15]);

    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSOX_0, LSM6DSOX_FUNC_CFG_ACCESS, LSM6DSOX_USER_BANK << 6);
  }
  else if ((AccInstance == IKS01A2_LSM6DSR_0) && (GyrInstance == IKS01A2_LSM6DSR_0))
  {
    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FUNC_CFG_ACCESS, LSM6DSR_EMBEDDED_FUNC_BANK << 6);

    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS1, &fsm_status[0]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS2, &fsm_status[1]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS3, &fsm_status[2]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS4, &fsm_status[3]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS5, &fsm_status[4]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS6, &fsm_status[5]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS7, &fsm_status[6]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS8, &fsm_status[7]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS9, &fsm_status[8]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS10, &fsm_status[9]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS11, &fsm_status[10]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS12, &fsm_status[11]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS13, &fsm_status[12]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS14, &fsm_status[13]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS15, &fsm_status[14]);
    (void)IKS01A2_MOTION_SENSOR_Read_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FSM_OUTS16, &fsm_status[15]);

    (void)IKS01A2_MOTION_SENSOR_Write_Register(IKS01A2_LSM6DSR_0, LSM6DSR_FUNC_CFG_ACCESS, LSM6DSR_USER_BANK << 6);
  }
  else
  {
    fsm_status[0] = 0;
...
static void DIL24_INT1_Force_Low(void)
{
  ...
  uint8_t value = 0x42;
  BSP_I2C1_WriteReg(LPS22HH_I2C_ADD_L, LPS22HH_IF_CTRL, &value, 1);
}
...
void TIM3_Init(void)
{
  const uint32_t tim_counter_clock = 1000000; /* TIM counter clock 1 MHz */
  uint32_t prescaler_value = (uint32_t)((SystemCoreClock / tim_counter_clock) - 1);
  uint32_t period = 1000000; /* Reset TIM counter after 1 second */
