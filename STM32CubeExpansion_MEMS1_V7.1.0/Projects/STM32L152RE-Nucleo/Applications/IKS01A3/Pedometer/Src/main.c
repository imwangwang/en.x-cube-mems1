...
/**
 * @mainpage Documentation for MotionPM package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionPM software is an add-on for the X-CUBE-MEMS1 software and provides
 * the real-time step count and cadence.
 ...
 *         expansion board to detect the steps and cadence and send it from a Nucleo
...
int main(void)
{
  ...
  */
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionPM_manager_get_version(lib_version, &lib_version_len);
...
static void PM_Data_Handler(TMsg *Msg)
{
  ...
  static MPM_output_t data_out_prev = {.Cadence = 0, .Nsteps = 0};
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].cadence = data_out.Cadence;
        DataByte[DataIndex].steps = data_out.Nsteps;
        DataIndex++;

        data_out_prev.Cadence = data_out.Cadence;
        data_out_prev.Nsteps = data_out.Nsteps;
        return;
      }
      ...
      /* Data changed */
      if (data_out.Cadence != data_out_prev.Cadence || data_out.Nsteps != data_out_prev.Nsteps)
      {
