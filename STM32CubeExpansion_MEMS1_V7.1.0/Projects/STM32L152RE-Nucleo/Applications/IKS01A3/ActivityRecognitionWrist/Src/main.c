...
static void AW_Data_Handler(TMsg *Msg)
{
  ...
  static MAW_activity_t activity_prev = MAW_NOACTIVITY;
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    BSP_LED_On(LED2);
    MotionAW_manager_run(&data_in, &activity, TimeStamp);
    BSP_LED_Off(LED2);
    ...
    else if (ProgramState == STANDALONE_MODE)
    {
      ...
      if (SessionNew == 1U)
      {
        ...
        DataByte[DataIndex].activity_type = (uint8_t)MAW_NOACTIVITY;
        DataIndex++;

        activity_prev = MAW_NOACTIVITY;
        return;
      }
