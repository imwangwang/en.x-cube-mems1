...
/**
 * @mainpage Documentation for MotionFD package of X-CUBE-MEMS1 Software for X-NUCLEO-IKS01Ax expansion boards
 *
 * @image html st_logo.png
 *
 * <b>Introduction</b>
 *
 * MotionFD software is an add-on for the X-CUBE-MEMS1 software and provides
 * the real-time user fall events detection.
 ...
 *         expansion board to recognize user fall and send it from a Nucleo
...
int main(void)
{
  ...
  */
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionFD_manager_get_version(lib_version, &lib_version_len);
  ...
  for (;;)
  {
    ...
    if (GuiModeRequest == 1U)
    {
      ...
      SensorsEnabled &= ~(ACCELEROMETER_SENSOR | PRESSURE_SENSOR);
    ...
    if (SensorReadRequest == 1U)
    {
      ...
      switch (ProgramState)
      {
        case GUI_MODE:
          ...
          /* Fall Detection specific part */
          FD_Data_Handler(&msg_dat);
...
static void Init_Sensors(void)
{
   ...
   *   - FS   = <-4g, 4g>
   */
  (void)IKS01A2_MOTION_SENSOR_SetOutputDataRate(IKS01A2_LSM6DSL_0, MOTION_ACCELERO, 100.0f);
...
/**
 * @brief  Fall Detection data handler
 * @param  Msg the Fall Detection data part of the stream
 * @retval None
 */
static void FD_Data_Handler(TMsg *Msg)
{
  MFD_input_t data_in = {.AccX = 0.0f, .AccY = 0.0f, .AccZ = 0.0f};
  static MFD_output_t data_out;
  static MFD_output_t data_out_prev = MFD_NOFALL;
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    {
      /* Add acceleration [mg] */
      data_in.AccX = (float)AccValue.x;
      data_in.AccY = (float)AccValue.y;
      data_in.AccZ = (float)AccValue.z;
      ...
      BSP_LED_On(LED2);
      MotionFD_manager_update(&data_in, &data_out);
      BSP_LED_Off(LED2);
      ...
      else if (ProgramState == STANDALONE_MODE)
      {
        ...
        if (SessionNew == 1U)
        {
          ...
          DataByte[DataIndex].fall = (uint8_t)MFD_NOFALL;
          DataIndex++;

          data_out_prev = MFD_NOFALL;
          return;
        }
        ...
        /* Data changed */
        if (data_out_prev != data_out)
        {
          /* Store new data in buffer */
          if (data_out == MFD_FALL)
          {
            RTC_Handler_DataStamp(&DataByte[DataIndex].date_time);
            DataByte[DataIndex].data_valid = 1; /* 0 if new session */
            DataByte[DataIndex].fall = (uint8_t)data_out;
            DataIndex++;
          }
