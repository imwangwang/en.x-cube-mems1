...
static void ID_Data_Handler(TMsg *Msg);
...
{
  char fw_id[] = {"9"}; /* FW ID for Unicleo-GUI */
  ...
  MotionID_manager_get_version(lib_version_string, &lib_version_len);
...
/* Private functions ---------------------------------------------------------*/
/**
  * @brief  Initialize the IntensityDetection application
  * @retval None
  */
static void MX_IntensityDetection_Init(void)
{
  ...
  /* OPTIONAL */
  /* Get library version */
  MotionID_manager_get_version(lib_version, &lib_version_len);
...
/**
  * @brief  Process of the IntensityDetection application
  * @retval None
  */
static void MX_IntensityDetection_Process(void)
{
...
static void Init_Sensors(void)
{
   ...
   *   - FS   = <-4g, 4g>
   */
  BSP_SENSOR_ACC_SetOutputDataRate(16.0f);
