...
static void MX_GestureRecognition_Init(void);
static void MX_GestureRecognition_Process(void);

static void Init_Sensors(void);
static void RTC_Handler(TMsg *Msg);
static void GR_Data_Handler(TMsg *Msg);
...
{
  char fw_id[] = {"2"}; /* FW ID for Unicleo-GUI */
  ...
  MotionGR_manager_get_version(lib_version_string, &lib_version_len);
...
/* Private functions ---------------------------------------------------------*/
/**
  * @brief  Initialize the GestureRecognition application
  * @retval None
  */
static void MX_GestureRecognition_Init(void)
{
  ...
  /* GestureRecognition API initialization function */
  MotionGR_manager_init();

  /* OPTIONAL */
  /* Get library version */
  MotionGR_manager_get_version(lib_version, &lib_version_len);
...
/**
  * @brief  Process of the GestureRecognition application
  * @retval None
  */
static void MX_GestureRecognition_Process(void)
{
  ...
  if (SensorReadRequest == 1U)
  {
    ...
    /* GestureRecognition specific part */
    GR_Data_Handler(&msg_dat);
...
/**
 * @brief  GestureRecognition data handler
 * @param  Msg the GestureRecognition data part of the stream
 * @retval None
 */
static void GR_Data_Handler(TMsg *Msg)
{
  ...
  if ((SensorsEnabled & ACCELEROMETER_SENSOR) == ACCELEROMETER_SENSOR)
  {
    ...
    /* Run GestureRecognition algorithm */
    BSP_LED_On(LED2);
    DWT_Start();
    MotionGR_manager_run(&data_in, &data_out);
