...
/**
  ...
  * @retval None
  */
void MX_LSM6DSO_Tilt_Init(void)
{
  /* Initialize LED */
  BSP_LED_Init(LED2);
...
/**
  ...
  * @retval None
  */
void MX_LSM6DSO_Tilt_Process(void)
{
...
[#if LSM6DSO_EVENT1_value == "INT1"][#-- ####################################### --]
    if (CUSTOM_MOTION_SENSOR_Enable_Tilt_Detection(CUSTOM_LSM6DSO_0, CUSTOM_MOTION_SENSOR_INT1_PIN) != BSP_ERROR_NONE)
[/#if][#-- ################################################################# --]
[#if LSM6DSO_EVENT1_value == "INT2"][#-- ####################################### --]
    if (CUSTOM_MOTION_SENSOR_Enable_Tilt_Detection(CUSTOM_LSM6DSO_0, CUSTOM_MOTION_SENSOR_INT2_PIN) != BSP_ERROR_NONE)
[/#if][#-- ################################################################# --]
    {
      Error_Handler();
    }
  ...
  {
    if (CUSTOM_MOTION_SENSOR_Disable_Tilt_Detection(CUSTOM_LSM6DSO_0) != BSP_ERROR_NONE)
    {
      Error_Handler();
    }
