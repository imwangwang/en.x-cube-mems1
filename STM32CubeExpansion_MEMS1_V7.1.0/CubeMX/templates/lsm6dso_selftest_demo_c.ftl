...
static int32_t LSM6DSO_G_SelfTest(void)
{
  ...
  for (i = 0; i < ST_REG_COUNT; i++)
  {
    if ((ret = CUSTOM_MOTION_SENSOR_Write_Register(CUSTOM_LSM6DSO_0, reg_addr[i], g_st_reg_values[i])) != BSP_ERROR_NONE)
    {
      return ret;
    }
  }
  ...
  /* Enable self-test */
  if ((ret = CUSTOM_MOTION_SENSOR_Set_SelfTest(CUSTOM_LSM6DSO_0, MOTION_GYRO, (uint8_t)LSM6DSO_GY_ST_POSITIVE)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSO_G_Get_Data(CUSTOM_MOTION_SENSOR_Axes_t *data)
{
  ...
  do
  {
    if ((ret = CUSTOM_MOTION_SENSOR_Get_DRDY_Status(CUSTOM_LSM6DSO_0, MOTION_GYRO, &status)) != BSP_ERROR_NONE)
    {
      return ret;
    }
  }
  ...
  /* Read accelero data */
  if ((ret = CUSTOM_MOTION_SENSOR_GetAxes(CUSTOM_LSM6DSO_0, MOTION_GYRO, data)) != BSP_ERROR_NONE)
  {
    return ret;
  }
