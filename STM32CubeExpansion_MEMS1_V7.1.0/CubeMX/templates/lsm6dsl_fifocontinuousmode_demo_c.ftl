...
/**
  ...
  * @retval None
  */
void MX_LSM6DSL_FIFOContinuousMode_Init(void)
{
  /* Initialize LED */
  BSP_LED_Init(LED2);
  ...
  (void)CUSTOM_MOTION_SENSOR_Init(CUSTOM_LSM6DSL_0, MOTION_GYRO);
...
/**
  ...
  * @retval None
  */
void MX_LSM6DSL_FIFOContinuousMode_Process(void)
{
  ...
  switch (DemoFifoStatus)
  {
    ...
    case STATUS_FIFO_RUN:
      ...
      if (CUSTOM_MOTION_SENSOR_FIFO_Get_Num_Samples(CUSTOM_LSM6DSL_0, &num_samples) != BSP_ERROR_NONE)
      {
        Error_Handler();
      }
    ...
    case STATUS_FIFO_DOWNLOAD:
      ...
      if (CUSTOM_MOTION_SENSOR_FIFO_Get_Full_Status(CUSTOM_LSM6DSL_0, &fifo_full_status) != BSP_ERROR_NONE)
      {
        Error_Handler();
      }
...
static int32_t LSM6DSL_FIFO_Demo_Config(void)
{
  ...
  if ((ret = CUSTOM_MOTION_SENSOR_SetOutputDataRate(CUSTOM_LSM6DSL_0, MOTION_GYRO, LSM6DSL_SAMPLE_ODR)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set gyro FIFO decimation */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Decimation(CUSTOM_LSM6DSL_0, MOTION_GYRO, (uint8_t)LSM6DSL_FIFO_GY_NO_DEC)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set FIFO ODR to highest value */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_ODR_Value(CUSTOM_LSM6DSL_0, LSM6DSL_FIFO_MAX_ODR)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set FIFO_FULL on INT1 */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_INT1_FIFO_Full(CUSTOM_LSM6DSL_0, ENABLE)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set FIFO watermark */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Watermark_Level(CUSTOM_LSM6DSL_0, FIFO_WATERMARK)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set FIFO depth to be limited to watermark threshold level  */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Stop_On_Fth(CUSTOM_LSM6DSL_0, ENABLE)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSL_FIFO_Set_Bypass_Mode(void)
{
  ...
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Mode(CUSTOM_LSM6DSL_0, (uint8_t)LSM6DSL_BYPASS_MODE)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSL_FIFO_Set_Continuous_Mode(void)
{
  ...
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Mode(CUSTOM_LSM6DSL_0, (uint8_t)LSM6DSL_STREAM_MODE)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSL_Read_All_FIFO_Data(void)
{
  ...
  /* Get num of unread FIFO samples before reading data */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Get_Num_Samples(CUSTOM_LSM6DSL_0, &samples_to_read)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSL_Read_Single_FIFO_Pattern_Cycle(uint16_t SampleIndex)
{
  ...
  for (i = 0; i <= 2; i++)
  {
    ...
    if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Get_Pattern(CUSTOM_LSM6DSL_0, &pattern)) != BSP_ERROR_NONE)
    {
      return ret;
    }

    /* Read single FIFO data (angular velocity in one axis) */
    if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Get_Axis(CUSTOM_LSM6DSL_0, MOTION_GYRO, &angular_velocity)) != BSP_ERROR_NONE)
    {
      return ret;
    }
