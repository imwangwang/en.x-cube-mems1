...
/**
  ...
  * @retval None
  */
void MX_LSM6DSL_FIFOLowPower_Init(void)
{
  /* Initialize LED */
  BSP_LED_Init(LED2);
...
/**
  ...
  * @retval None
  */
void MX_LSM6DSL_FIFOLowPower_Process(void)
{
  ...
  switch (DemoFifoStatus)
  {
    ...
    case STATUS_FIFO_DOWNLOAD:
      ...
      if (CUSTOM_MOTION_SENSOR_FIFO_Set_Mode(CUSTOM_LSM6DSL_0, (uint8_t)LSM6DSL_BYPASS_MODE) != BSP_ERROR_NONE)
      {
        Error_Handler();
      }
      DemoFifoStatus = STATUS_SLEEP;
    ...
    case STATUS_SLEEP:
      ...
      if (CUSTOM_MOTION_SENSOR_FIFO_Set_Mode(CUSTOM_LSM6DSL_0, (uint8_t)LSM6DSL_FIFO_MODE) != BSP_ERROR_NONE)
      {
        Error_Handler();
      }
      Sleep_Mode();
...
static int32_t LSM6DSL_FIFO_Demo_Config(void)
{
  ...
  if ((ret = CUSTOM_MOTION_SENSOR_SetOutputDataRate(CUSTOM_LSM6DSL_0, MOTION_ACCELERO, LSM6DSL_SAMPLE_ODR)) != BSP_ERROR_NONE)
  {
    return ret;
  }

  /* Set accelero FIFO decimation */
  if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Set_Decimation(CUSTOM_LSM6DSL_0, MOTION_ACCELERO, (uint8_t)LSM6DSL_FIFO_XL_NO_DEC)) != BSP_ERROR_NONE)
  {
    return ret;
  }
...
static int32_t LSM6DSL_Read_Single_FIFO_Pattern_Cycle(uint16_t SampleIndex)
{
  ...
  for (i = 0; i <= 2; i++)
  {
    ...
    if ((ret = CUSTOM_MOTION_SENSOR_FIFO_Get_Axis(CUSTOM_LSM6DSL_0, MOTION_ACCELERO, &acceleration)) != BSP_ERROR_NONE)
    {
      return ret;
    }
