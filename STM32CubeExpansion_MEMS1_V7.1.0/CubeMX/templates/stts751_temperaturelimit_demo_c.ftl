...
/**
  ...
  * @retval None
  */
void MX_STTS751_TemperatureLimit_Init(void)
{
  /* Initialize LED */
  BSP_LED_Init(LED2);
  ...
  /* Sensor init */
  (void)CUSTOM_ENV_SENSOR_Init(CUSTOM_STTS751_0, ENV_TEMPERATURE);

  /* Setup output data rate */
  (void)CUSTOM_ENV_SENSOR_SetOutputDataRate(CUSTOM_STTS751_0, ENV_TEMPERATURE, 4.0f);

  /* Setup temperature limits */
  (void)CUSTOM_ENV_SENSOR_Set_Low_Temperature_Threshold(CUSTOM_STTS751_0, LOW_TEMPERATURE_THRESHOLD);
  (void)CUSTOM_ENV_SENSOR_Set_High_Temperature_Threshold(CUSTOM_STTS751_0, HIGH_TEMPERATURE_THRESHOLD);

  /* Enable interrupt on STTS751 EVENT pin */
  (void)CUSTOM_ENV_SENSOR_Set_Event_Pin(CUSTOM_STTS751_0, 1U);

  /* Read status register to clear previous value */
  (void)CUSTOM_ENV_SENSOR_Get_Temperature_Limit_Status(CUSTOM_STTS751_0, NULL, NULL, NULL);
...
/**
  ...
  * @retval None
  */
void MX_STTS751_TemperatureLimit_Process(void)
{
  ...
  if (MemsEventDetected != 0U)
  {
    ...
    if (CUSTOM_ENV_SENSOR_Get_Temperature_Limit_Status(CUSTOM_STTS751_0, &HighTemp, &LowTemp, NULL) == BSP_ERROR_NONE)
    {
      /* If high or low limit was reached read actual temperature and blink with the LED */
      if ((HighTemp != 0U) || (LowTemp != 0U))
      {
        (void)CUSTOM_ENV_SENSOR_GetValue(CUSTOM_STTS751_0, ENV_TEMPERATURE, &Temperature);
